...
/**
 * Même logique que public/admin/api/scraping/search.php — chemin bootstrap adapté
 * quand le vhost sert /admin depuis site/admin/ (et non public/admin/).
 */
ob_start();
...
if (trim((string) EXP_SUPABASE_URL) === '' || trim((string) EXP_SUPABASE_KEY) === '') {
    echo json_encode([
        'success' => false,
        'message' => 'Configuration Supabase absente : renseignez SUPABASE_URL et SUPABASE_ANON_KEY dans le fichier .env.',
    ]);
    exit;
}
...
if (!empty($extIds)) {
    try {
        ...
        error_log('[scraping search] already_imported: ' . $e->getMessage());
        // Sans colonnes 038, on renvoie quand même les biens Supabase (pas de marquage importé)
    }
}
