...
contactBien?.addEventListener('submit', async e => {
  ...
  try {
    ...
    if (json.success) {
      contactBien.innerHTML = `<p class="form-feedback--inline" role="status">Message envoyé ! ${ADVISOR_NAME} vous contactera sous 24h.</p>`;
    } else {
      btn.disabled = false;
      btn.textContent = 'Envoyer';
      window.siteToast?.(json.message || 'Une erreur est survenue.', 'error');
    }
  } catch {
    btn.disabled = false;
    btn.textContent = 'Envoyer';
    window.siteToast?.('Impossible de joindre le serveur. Réessayez dans un instant.', 'error');
  }
});
