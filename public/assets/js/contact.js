  ...
  try {
    ...
    if (json.success) {
      contactForm.closest('.contact-form-box').innerHTML = `
        <div class="form-feedback" role="status">
          <div class="form-feedback__icon" aria-hidden="true">✓</div>
          <h2>Message envoyé !</h2>
          <p>${ADVISOR_NAME} vous répondra dans les plus brefs délais, généralement sous 24h.</p>
        </div>`;
    } else {
      btn.disabled = false;
      btn.textContent = original;
      window.siteToast?.(json.message || 'Une erreur est survenue. Veuillez réessayer.', 'error');
    }
  } catch {
    btn.disabled = false;
    btn.textContent = original;
    window.siteToast?.('Impossible de joindre le serveur. Vérifiez votre connexion et réessayez.', 'error');
  }
});
