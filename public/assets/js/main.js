...
/**
 * Notification toast alignée sur le thème (remplace les alert() navigateur).
 * @param {string} message
 * @param {'success'|'error'|'info'|'warning'|'danger'} [type='info']
 * @param {number} [durationMs=4800] — 0 = pas d’auto-fermeture
 */
function siteToastEnsureStack() {
...
function siteToastIconLabel(type) {
...
window.siteToast = function siteToast(message, type = 'info', durationMs = 4800) {
  const stack = siteToastEnsureStack();
  const normalized = type === 'danger' ? 'error' : type;
  const toast = document.createElement('div');
  toast.className = 'site-toast site-toast--' + normalized;

  const live = normalized === 'error' ? 'assertive' : 'polite';
  ...
  icon.textContent = siteToastIconLabel(type);
