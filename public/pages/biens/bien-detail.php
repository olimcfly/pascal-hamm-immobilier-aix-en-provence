...
// Le routeur passe slug via page() (extract) ; l’ancien front utilisait $GLOBALS['bienSlug'].
...
$b['advisor_photo'] = trim((string) setting('advisor_photo', DEFAULT_ADVISOR_PHOTO_URL));
...
$bienPublicUrl = url('biens/' . (string) ($b['slug'] ?? ''));
        ...
        <div class="bien-detail-layout">
            ...
            <div class="bien-detail-main">
                ...
                <section class="bien-map" id="localisation">
                    ...
                    <!-- Points d'intérêt locaux (Pays d'Aix) -->
                    <div class="bien-poi-grid">
                        <?php
                        $pois = [
                            ['icon' => 'fa-graduation-cap', 'label' => 'Université / grandes écoles'],
                            ['icon' => 'fa-train',          'label' => 'Gare TGV Aix-en-Provence'],
                            ['icon' => 'fa-bus',            'label' => 'Bus Aix en Bus / Lignes locales'],
                            ['icon' => 'fa-shopping-bag',   'label' => 'Cours Mirabeau & centre-ville'],
                            ['icon' => 'fa-plane',          'label' => 'Aéroport Marseille-Provence'],
                            ['icon' => 'fa-road',           'label' => 'A8 / A51 — accès autoroutier'],
                        ];
                        foreach ($pois as $poi): ?>
                ...
                <section class="bien-legal">
                    ...
                    <p class="bien-legal__ref">
                        ...
                        indépendant rattaché au réseau EXP France.
            ...
            <aside class="bien-detail-sidebar">
                ...
                <div class="sidebar-card sidebar-card--contact-form">
                    ...
                    <?php
                    // Traitement formulaire contact rapide (succès → redirect /biens/{slug}/merci)
                    $formError = '';
                    ...
                    if ($_SERVER['REQUEST_METHOD'] === 'POST'
                        ...
                        if (!hash_equals($_SESSION['csrf_token'] ?? '', $token)) {
                            ...
                            $formError = 'Veuillez accepter la politique de confidentialité pour envoyer votre demande.';
                        } else {
                            require_once ROOT_PATH . '/core/services/MailService.php';

                            $nameParts = preg_split('/\s+/', $nom, 2, PREG_SPLIT_NO_EMPTY);
                            $fn = $nameParts[0] ?? $nom;
                            $ln = $nameParts[1] ?? '';

                            $refLine = 'Réf. : ' . ($b['reference'] ?? '') . ' (id ' . (int) ($b['id'] ?? 0) . ')';
                            $notes   = $refLine . "\n\n" . $message;

                            $emailSeqMeta = [
                                'bien_titre' => (string) ($b['titre'] ?? ''),
                                'bien_url' => $bienPublicUrl,
                                'bien_reference' => (string) ($b['reference'] ?? ''),
                                'bien_ville' => (string) ($b['ville'] ?? ''),
                                'bien_type' => (string) ($b['type_label'] ?? $b['type_bien'] ?? ''),
                                'bien_prix' => formatPrice((int) ($b['prix'] ?? 0)),
                            ];
                            ...
                            $payload = [
                                ...
                                'email'       => $email,
                                'phone'       => $tel,
                                'intent'      => 'Fiche bien — ' . ($b['titre'] ?? ''),
                                'property_type' => (string) ($b['type_bien'] ?? ''),
                                'property_address' => trim(implode(' ', array_filter([
                                    trim((string) ($b['adresse'] ?? '')),
                                    trim(((string) ($b['code_postal'] ?? '')) . ' ' . ((string) ($b['ville'] ?? ''))),
                                ], static fn ($x) => $x !== ''))),
                                'notes'       => $notes,
                                'consent'     => 1,
                                'metadata'    => [
                                    'bien_id'        => (int) ($b['id'] ?? 0),
                                    'bien_slug'      => (string) ($b['slug'] ?? ''),
                                    'bien_reference' => (string) ($b['reference'] ?? ''),
                                ],
                                'email_sequence_trigger' => 'bien-detail',
                                'email_sequence_metadata' => $emailSeqMeta,
                            ];

                            try {
                                $leadId = LeadService::capture($payload);
                            } catch (Throwable $e) {
                                error_log('[bien-detail contact] CRM: ' . $e->getMessage());
                                $formError = 'Enregistrement impossible pour le moment. Merci de réessayer ou de nous appeler.';
                                $leadId = 0;
                            }

                            if ($formError === '' && $leadId > 0) {
                                $adminTo = trim((string) setting('advisor_email', ''));
                                if ($adminTo === '' && defined('APP_EMAIL')) {
                                    $adminTo = trim((string) constant('APP_EMAIL'));
                                }
                                if ($adminTo === '') {
                                    $adminTo = trim((string) (setting('smtp_from', '') ?: ($_ENV['SMTP_FROM_EMAIL'] ?? '')));
                                }

                                $bienUrl = $bienPublicUrl;
                                $siteName = defined('APP_NAME') ? (string) APP_NAME : '';
                                $bienT = htmlspecialchars($emailSeqMeta['bien_titre'] ?: 'Annonce', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
                                $bienUrlH = htmlspecialchars($bienUrl, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');

                                if ($adminTo !== '' && filter_var($adminTo, FILTER_VALIDATE_EMAIL)) {
                                    $adminBody = "Nouvelle demande depuis une fiche bien.\n\n"
                                        . "Bien : " . $emailSeqMeta['bien_titre'] . "\n"
                                        . $refLine . "\n"
                                        . "Prix : " . $emailSeqMeta['bien_prix'] . "\n"
                                        . "Ville : " . $emailSeqMeta['bien_ville'] . "\n"
                                        . "URL : {$bienUrl}\n\n"
                                        . "Nom : {$nom}\nEmail : {$email}\nTél : {$tel}\n\nMessage :\n{$message}\n\n—\nLead CRM #{$leadId}";
                                    $adminHtml = '<p style="font-family:system-ui,sans-serif;font-size:15px;line-height:1.55;color:#1a1a2e;">'
                                        . '<strong>Nouvelle demande</strong> — fiche bien</p>'
                                        . '<p style="font-family:system-ui,sans-serif;font-size:15px;">'
                                        . '<strong>' . $bienT . '</strong><br>'
                                        . 'Réf. ' . htmlspecialchars($emailSeqMeta['bien_reference'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
                                        . ' · ' . htmlspecialchars($emailSeqMeta['bien_prix'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
                                        . '<br>' . htmlspecialchars($emailSeqMeta['bien_type'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
                                        . ' à ' . htmlspecialchars($emailSeqMeta['bien_ville'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
                                        . '</p>'
                                        . '<p><a href="' . $bienUrlH . '">' . $bienUrlH . '</a></p>'
                                        . '<p style="font-family:system-ui,sans-serif;font-size:15px;">'
                                        . '<strong>Contact</strong><br>'
                                        . htmlspecialchars($nom, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '<br>'
                                        . '<a href="mailto:' . htmlspecialchars($email, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '">'
                                        . htmlspecialchars($email, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</a><br>'
                                        . htmlspecialchars($tel, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</p>'
                                        . '<p style="font-family:system-ui,sans-serif;font-size:15px;">'
                                        . '<strong>Message</strong><br>' . nl2br(htmlspecialchars($message, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')) . '</p>'
                                        . '<p style="color:#666;font-size:13px;">Lead CRM #' . (int) $leadId . '</p>';
                                    try {
                                        MailService::send(
                                            $adminTo,
                                            'Nouveau contact — ' . ($emailSeqMeta['bien_titre'] !== '' ? $emailSeqMeta['bien_titre'] : 'Bien'),
                                            $adminBody,
                                            $adminHtml
                                        );
                                    } catch (Throwable $e) {
                                        error_log('[bien-detail contact] admin mail: ' . $e->getMessage());
                                    }
                                }

                                $prenom = $fn !== '' ? $fn : 'Bonjour';
                                $advisorN = defined('ADVISOR_NAME') ? ADVISOR_NAME : 'Notre équipe';
                                $replyText = 'Bonjour ' . $prenom . ",\n\n"
                                    . 'Nous avons bien reçu votre demande concernant « ' . $emailSeqMeta['bien_titre'] . " »\n"
                                    . '(' . $emailSeqMeta['bien_type'] . ' à ' . $emailSeqMeta['bien_ville'] . ', ' . $emailSeqMeta['bien_prix'] . ").\n\n"
                                    . "Lien de l'annonce : {$bienUrl}\n\n"
                                    . $advisorN
                                    . " vous recontactera très prochainement — en général sous 24 h (jours ouvrés). Une seconde série d'emails vous donnera des conseils liés à cette recherche.\n\n"
                                    . "Cordialement,\n" . ($siteName !== '' ? $siteName : '');
                                $replyHtml = '<p style="font-family:Georgia,serif;font-size:16px;line-height:1.6;color:#1a1a2e;">Bonjour '
                                    . htmlspecialchars($prenom, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . ',</p>'
                                    . '<p style="font-family:Georgia,serif;font-size:16px;line-height:1.6;">Nous avons bien reçu votre demande concernant <strong>« ' . $bienT . ' »</strong> '
                                    . '(' . htmlspecialchars($emailSeqMeta['bien_type'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . ' à '
                                    . htmlspecialchars($emailSeqMeta['bien_ville'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . ', '
                                    . htmlspecialchars($emailSeqMeta['bien_prix'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . ').</p>'
                                    . '<p style="font-family:Georgia,serif;font-size:16px;">'
                                    . '<a href="' . $bienUrlH . '">Voir l’annonce</a></p>'
                                    . '<p style="font-family:Georgia,serif;font-size:16px;">'
                                    . htmlspecialchars($advisorN, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
                                    . ' vous recontactera très prochainement — en général sous 24 h (jours ouvrés). Vous recevrez aussi de courts emails pour vous aider dans votre recherche.</p>'
                                    . '<p style="font-family:Georgia,serif;">Cordialement,<br>'
                                    . htmlspecialchars($siteName !== '' ? $siteName : 'L’équipe', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</p>';
                                try {
                                    MailService::send(
                                        $email,
                                        ($siteName !== '' ? $siteName . ' — ' : '') . 'Accusé de réception',
                                        $replyText,
                                        $replyHtml
                                    );
                                } catch (Throwable $e) {
                                    error_log('[bien-detail contact] prospect mail: ' . $e->getMessage());
                                }

                                header('Location: ' . url('biens/' . rawurlencode((string) ($b['slug'] ?? '')) . '/merci'), true, 303);
                                exit;
                            }
                        }
                    }
                    ...
                    <form method="POST"
                          action="<?= htmlspecialchars(url('biens/' . (string) ($b['slug'] ?? ''))) ?>"
                          class="contact-bien-form"
                          novalidate>
