...
$pageDescription = 'Découvrez les biens immobiliers à vendre sur Aix-en-Provence et le Pays d’Aix (13).';
...
<section class="hero hero--premium">
    ...
    <div class="container">
        <div class="hero__content">
            ...
            <h1>Maisons et appartements à vendre — Aix &amp; Pays d’Aix</h1>
            ...
            <?php if (!empty($biens)): ?>
                <?php foreach ($biens as $bien): ?>
                    <article class="card property-card-premium">
                        <?php
                            $bSlug = trim((string) ($bien['slug'] ?? ''));
                            $bienFicheUrl = $bSlug !== '' ? '/biens/' . rawurlencode($bSlug) : '/bien.php?id=' . (int) ($bien['id'] ?? 0);
                        ?>
                        <?php
                            $ph_maison = '/assets/images/placeholder.php?type=maison&surface=' . (int)($bien['surface'] ?? 0) . '&pieces=' . (int)($bien['pieces'] ?? 0);
                            $src_maison = (!empty($bien['photo_principale']) && strpos($bien['photo_principale'], 'default.jpg') === false)
                                ? e($bien['photo_principale'])
                                : ((!empty($bien['image'])) ? '/uploads/' . e($bien['image']) : $ph_maison);
                        ?>
                        <img
                            class="card__img"
                            src="<?= $src_maison ?>"
                        ...
                        <div class="card__body">
                            ...
                            <a href="<?= htmlspecialchars($bienFicheUrl) ?>" class="btn btn--primary btn--sm">
                                Voir le bien
                            </a>
...
<section class="cta-banner">
    <div class="container">
        <h2>Vous avez un projet immobilier sur Aix ou le Pays d’Aix ?</h2>
